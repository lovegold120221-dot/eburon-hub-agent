...
[Beatrice Agent](https://beatrice-agent.nousresearch.com/) is a sophisticated autonomous agent that lives on your server, accessed via a terminal or messaging apps, that remembers what it learns and gets more capable the longer it runs.

Eburon Hub is a lightweight, dark-themed web app interface in your browser for [Beatrice Agent](https://beatrice-agent.nousresearch.com/).
...
**Eburon Hub Control Center** (launcher at the sidebar bottom).

<img width="2448" height="1748" alt="Eburon Hub — three-panel layout" src="https://github.com/user-attachments/assets/6bf8af4c-209d-441e-8b92-6515d7a0c369" />
...
This gives you nearly **1:1 parity with Eburon Hub CLI from a convenient web UI** which you can access securely through an SSH tunnel from your Eburon Hub setup. Single command to start this up, and a single command to SSH tunnel for access on your computer. Every single part of the web UI uses your existing Beatrice agent and existing models, without requiring any additional setup.
...
Eburon Hub retains context across sessions, runs scheduled jobs while you're offline, and gets
smarter about your environment the longer it runs. It uses your existing Beatrice agent setup,
...
- **Persistent memory** — user profile, agent notes, and a skills system that saves reusable
  procedures; Eburon Hub learns your environment and does not have to relearn it
...
- **Self-improving skills** — Eburon Hub writes and saves its own skills automatically from experience;
...
| | OpenClaw | Claude Code | Codex CLI | OpenCode | Eburon Hub |
|---|---|---|---|---|---|
...
with memory, cron, and messaging. The key differences: Eburon Hub writes and saves its own skills
automatically as a core behavior (OpenClaw's skill system centers on a community marketplace);
Eburon Hub is more stable across updates (OpenClaw has documented release regressions and ClawHub
has had security incidents involving malicious skills); and Eburon Hub runs natively in the Python
...
```bash
git clone https://github.com/nesquena/eburon-webui.git eburon-webui
cd eburon-webui
...
```bash
./ctl.sh start              # background daemon, PID at ~/.eburon/webui.pid
./ctl.sh status             # PID, uptime, bound host/port, log path, /health
./ctl.sh logs --lines 100   # tail ~/.eburon/webui.log
./ctl.sh restart
./ctl.sh stop
```

`ctl.sh start` runs the bootstrap in foreground/no-browser mode behind the daemon wrapper, writes logs to `~/.eburon/webui.log`, and respects `.env` plus inline overrides such as `EBURON_WEBUI_HOST=0.0.0.0 ./ctl.sh start`.

The bootstrap will:

1. Detect Beatrice Agent and, if missing, attempt the official installer (`curl -fsSL https://raw.githubusercontent.com/NousResearch/beatrice-agent/main/scripts/install.sh | bash`).
...
If provider setup is still incomplete after install, the onboarding wizard will point you to finish it with `eburon model` instead of trying to replicate the full CLI setup in-browser.
...
```bash
git clone https://github.com/nesquena/eburon-webui
cd eburon-webui
...
The container auto-detects your UID/GID from the mounted `~/.eburon` volume so files written by the agent stay readable by you on the host.

To enable password protection (required if you expose the port outside `127.0.0.1`):

```bash
echo "EBURON_WEBUI_PASSWORD=change-me-to-something-strong" >> .env
...
```bash
docker pull ghcr.io/nesquena/eburon-webui:latest
docker run -d \
  -e WANTED_UID=$(id -u) -e WANTED_GID=$(id -g) \
  -v ~/.eburon:/home/eburonwebui/.eburon \
  -e EBURON_WEBUI_STATE_DIR=/home/eburonwebui/.eburon/webui \
  -v ~/workspace:/workspace \
  -p 127.0.0.1:8787:8787 \
  ghcr.io/nesquena/eburon-webui:latest
```

### Build locally

```bash
docker build -t eburon-webui .
docker run -d \
  -e WANTED_UID=$(id -u) -e WANTED_GID=$(id -g) \
  -v ~/.eburon:/home/eburonwebui/.eburon \
  -e EBURON_WEBUI_STATE_DIR=/home/eburonwebui/.eburon/webui \
...
> **Known limitation (#681)**: in the two-container setup, tools triggered from the WebUI run in the **WebUI container**, not the agent container. If you need git/node/etc. on the WebUI's filesystem, either use the single-container setup, extend the WebUI Dockerfile, or use the community [all-in-one image](https://github.com/sunnysktsang/eburon-suite).
...
| `.env: permission denied` (#1389) | `fix_credential_permissions()` enforced 0600 | Set `EBURON_SKIP_CHMOD=1` in `.env` |
...
| WebUI can't find agent source | `beatrice-agent-src` volume misconfigured | Use the named volumes from compose files as-is |
| Podman shared `.eburon` fails | Podman 3.4 `keep-id` limitation | Use Podman 4+ or single-container |
...
> and set `EBURON_WEBUI_PASSWORD` to enable authentication.
...
|---|---|
| Beatrice agent dir | `EBURON_WEBUI_AGENT_DIR` env, then `~/.eburon/beatrice-agent`, then sibling `../beatrice-agent` |
| Python executable | Agent venv first, then `.venv` in this repo, then system `python3` |
| State directory | `EBURON_WEBUI_STATE_DIR` env, then `~/.eburon/webui-mvp` |
| Default workspace | `EBURON_WEBUI_DEFAULT_WORKSPACE` env, then `~/workspace`, then state dir |
| Port | `EBURON_WEBUI_PORT` env or first argument, default `8787` |
...
```bash
export EBURON_WEBUI_AGENT_DIR=/path/to/beatrice-agent
export EBURON_WEBUI_PYTHON=/path/to/python
export EBURON_WEBUI_PORT=9000
export EBURON_WEBUI_AUTO_INSTALL=1  # enable auto-install of agent deps (disabled by default)
./start.sh
```
...
```bash
EBURON_WEBUI_AGENT_DIR=/custom/path ./start.sh 9000
```
...
|---|---|---|
| `EBURON_WEBUI_AGENT_DIR` | auto-discovered | Path to the beatrice-agent checkout |
| `EBURON_WEBUI_PYTHON` | auto-discovered | Python executable |
| `EBURON_WEBUI_HOST` | `127.0.0.1` | Bind address (`0.0.0.0` for all IPv4, `::` for all IPv6, `::1` for IPv6 loopback) |
| `EBURON_WEBUI_PORT` | `8787` | Port |
| `EBURON_WEBUI_STATE_DIR` | `~/.eburon/webui-mvp` | Where sessions and state are stored |
| `EBURON_WEBUI_DEFAULT_WORKSPACE` | `~/workspace` | Default workspace |
| `EBURON_WEBUI_DEFAULT_MODEL` | `openai/gpt-5.4-mini` | Default model |
| `EBURON_WEBUI_PASSWORD` | *(unset)* | Set to enable password authentication |
| `EBURON_WEBUI_EXTENSION_DIR` | *(unset)* | Optional local directory served at `/extensions/`; must point to an existing directory before extension injection is enabled |
| `EBURON_WEBUI_EXTENSION_SCRIPT_URLS` | *(unset)* | Optional comma-separated same-origin script URLs to inject; see [WebUI Extensions](docs/EXTENSIONS.md) |
| `EBURON_WEBUI_EXTENSION_STYLESHEET_URLS` | *(unset)* | Optional comma-separated same-origin stylesheet URLs to inject; see [WebUI Extensions](docs/EXTENSIONS.md) |
| `EBURON_HOME` | `~/.eburon` | Base directory for Eburon Hub state (affects all paths) |
| `EBURON_CONFIG_PATH` | `~/.eburon/config.yaml` | Path to Eburon Hub config file |
...
Eburon Hub on a VPS or remote server, use an SSH tunnel from your local machine:
...
The Eburon Hub is fully responsive with a mobile-optimized layout
...
```bash
EBURON_WEBUI_HOST=0.0.0.0 EBURON_WEBUI_PASSWORD=your-secret ./start.sh
```
...
> **Tip:** If using Docker, set `EBURON_WEBUI_HOST=0.0.0.0` in your
> `docker-compose.yml` environment (already the default) and set
> `EBURON_WEBUI_PASSWORD`.
...
```bash
cd /path/to/beatrice-agent          # or wherever sys.path can find Eburon Hub modules
EBURON_WEBUI_PORT=8787 venv/bin/python /path/to/eburon-webui/server.py
```

Note: use the agent venv Python (or any Python environment that has the Beatrice agent dependencies installed). System Python will be missing `openai`, `httpx`, and other required packages.
...
Tests discover the repo and the Beatrice agent dynamically -- no hardcoded paths.
...
```bash
/path/to/beatrice-agent/venv/bin/python -m pytest tests/ -v
```
...
- Multi-provider model support -- any Eburon Hub API provider (OpenAI, Anthropic, Google, DeepSeek, Nous Portal, OpenRouter, MiniMax, Z.AI); dynamic model dropdown populated from configured keys
...
- CLI session bridge -- CLI sessions from beatrice-agent's SQLite store appear in the sidebar with a gold "cli" badge; click to import with full history and reply normally
...
- Enable via `EBURON_WEBUI_PASSWORD` env var or Settings panel
...
- **Eburon Hub Control Center** (sidebar launcher button) -- Conversation tab (export/import/clear), Preferences tab (model, send key, theme, language, all toggles), System tab (version, password)
...
api/
  ...
  profiles.py           Profile state management, eburon_cli wrapper (~411 lines)
...
State lives outside the repo at `~/.eburon/webui-mvp/` by default
(sessions, workspaces, settings, projects, last_workspace). Override with `EBURON_WEBUI_STATE_DIR`.

---

## Docs

- `HERMES.md` -- why Eburon Hub, mental model, and detailed comparison to Claude Code / Codex / OpenCode / Cursor
...
Eburon Hub is built with help from the open-source community. Every PR — whether merged directly or incorporated via batch release — shapes the project, and we're grateful to everyone who has taken the time to contribute.
...
```
git@github.com:nesquena/eburon-webui.git
```
